...
#        
#        for i,let in enumerate(lower[:26]):
...
#                dict[let] = lower[i]
#        
#        for i,let in enumerate(upper[:26]):
#            if let in VOWELS_UPPER:
#                dict[let] = vowels_permutation[i]
#            else:
#                dict[let] = let
...
#        
        for let in CONSONANTS_LOWER+CONSONANTS_UPPER:
            dict[let] = let
...
#            dict[let] = let
    ...
    def apply_transpose(self, transpose_dict):
        ...
        dict = transpose_dict
        # dict = self.build_transpose_dict()
        
        en_message = []
        for let in self.message_text:
            en_message.append(dict[let])
...
class EncryptedSubMessage(SubMessage):
    ...
    def decrypt_message(self):
        ...
        for perm in perm_list:
            ...
            transpose_dict_list.append(dict)
        
        
        de_msg_list =[]
        
        for dict in transpose_dict_list:
            
            de_msg = self.apply_transpose(dict)
            de_msg_list.append(de_msg)
        
        test =[]
        big_test=[]
        word_list = self.get_valid_words()
        for msg in de_msg_list:
            de_words = msg.split()
        ...
        for tup in big_test:
            if tup[0]==best_choice[0] and tup[1] not in possible_de_msg:
                possible_de_msg.append(tup[1])
        de_str = ''
        for msg in possible_de_msg:
            de_str = de_str+','+msg
        return de_str[1:]
